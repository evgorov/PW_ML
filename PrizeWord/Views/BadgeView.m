...
-(void)initWithPuzzle:(PuzzleData *)puzzle andNumber:(int)number
{
    if (self)
    {
        ...
        if (puzzle.progress < 1)
        {
            ...
            imgProgress.frame = CGRectMake(imgProgress.frame.origin.x, imgProgress.frame.origin.y + 1, _puzzle.progress * ([AppDelegate currentDelegate].isIPad ? 56 : 46), imgProgress.frame.size.height);
        }
        else
        {
            lblScore.hidden = NO;
