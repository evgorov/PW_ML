...
public class MagicTextView extends TextView {
	...
	public void init(AttributeSet attrs){
		...
		if(attrs != null){
			...
			if(a.hasValue(R.styleable.MagicTextView_mgbackground)){
				Drawable background = a.getDrawable(R.styleable.MagicTextView_mgbackground);
				if(background != null){
					this.setBackgroundDrawable(background);
				}else{
					this.setBackgroundColor(a.getColor(R.styleable.MagicTextView_mgbackground, 0xff000000));
				}
			}
	...
	public void onDraw(Canvas canvas){
		...
		if(innerShadows.size() > 0){
			...
			for(Shadow shadow : innerShadows){
				...
				this.setTextColor(restoreColor);
				this.setShadowLayer(shadow.r,shadow.dx,0,0);
			}
		}
