...
public class NotifyAboutBuysServerTask implements DbService.IDbTask {
    ...
    public Bundle execute(@Nonnull DbService.DbTaskEnv env) {
        ...
        else
        {
            ...
            RestPuzzleSet.RestPuzzleSetsHolder holder = loadRestPuzzleSetFromInternet(env.context, setServerId, receiptData);
        }
        return null;
    }

    private @Nullable
    RestPuzzleSet.RestPuzzleSetsHolder loadRestPuzzleSetFromInternet(@Nonnull Context context, @Nonnull String setServerId, @Nonnull String receiptData)
    {
        try
        {
