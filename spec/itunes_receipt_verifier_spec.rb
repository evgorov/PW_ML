...
describe ItunesReceiptVerifier do
  ...
  it 'should verify reciepts' do
    VCR.use_cassette('itunes_receipt_verifier') do
      lambda {
        ItunesReceiptVerifier.verify!(Redis.new, receipt_data, 1, 'ru.aipmedia.prizeword.2013_2_silver2_5')
  ...
  it 'should raise on bad receipts' do
    VCR.use_cassette('itunes_receipt_verifier_bad') do
      lambda {
        ItunesReceiptVerifier.verify!(Redis.new, 'bad_receipt', 1, 'ru.aipmedia.prizeword.2013_2_silver2_5')
      }.should raise_error(ItunesReceiptVerifier::ItunesReceiptError)
    end
  end
  ...
  it 'should raise error on invalid user_id' do
    VCR.use_cassette('itunes_receipt_verifier') do
      lambda {
        ItunesReceiptVerifier.verify!(Redis.new, receipt_data, 1, 'ru.aipmedia.prizeword.2013_2_silver2_5')
        ItunesReceiptVerifier.verify!(Redis.new, receipt_data, 2, 'ru.aipmedia.prizeword.2013_2_silver2_5')
